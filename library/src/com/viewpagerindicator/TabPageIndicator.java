...
public class TabPageIndicator extends HorizontalScrollView implements PageIndicator {
    ...
    private final OnClickListener mTabClickListener = new OnClickListener() {
        public void onClick(View view) {
            ...
            if (view instanceof TabView){
                newSelected = ((TabView)view).getIndex();
            } else {
    ...
    */
    ...
    public void setTabIconLocation (int newLocation){
        if (location > ICON_ONLY || location < LOCATION_LEFT)
    ...
    private void addTab(int index, CharSequence text, int iconResId) {
        View tabView = null;
        if (location == ICON_ONLY) {
            tabView = new TabImageView(getContext());
            ((TabImageView)tabView).mIndex = index;
            ((TabImageView)tabView).setImageResource(iconResId);
        } else {
            tabView = new TabView(getContext());
            ((TabView)tabView).mIndex = index;
            ((TabView)tabView).setText(text);
            if (iconResId != 0) {
                drawables[location] = iconResId;
                ((TabView)tabView).setCompoundDrawablesWithIntrinsicBounds(drawables[0],
