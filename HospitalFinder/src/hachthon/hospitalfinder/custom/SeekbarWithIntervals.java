...
//            alignIntervals();
    ...
    private void alignIntervalsInBetween(int maximumWidthOfEachInterval) {
        ...
        for (int index = 1; index < (getRelativeLayout().getChildCount() - 1); index++) {
            ...
            int leftPadding = Math.round(maximumWidthOfEachInterval - (widthOfText / 2)
                    - (widthOfPreviousIntervalsText / 2));
    ...
    private LinearLayout getRelativeLayout() {
        if (RelativeLayout == null) {
            RelativeLayout = (LinearLayout) findViewById(R.id.intervals);
        }
